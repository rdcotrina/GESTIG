                            ...
                            if (JSON.stringify(cont).indexOf(v.id_menu) == -1) {
                                ...
                                if (v.evt_ajax != 'not') {
                                    ...
                                    $(`#li_${v.id_menu}`).find('a').click(function () {
                                        ...
                                        eval(Tools.htmlEntities(v.evt_ajax));
                                    });
                                    selfmenu = $.grep(oSettings.data, function (e) {
                                        return e.id_rolmenu == v.id_rolmenu && e.access == 1;
                                    });
                                    /*agregar botones del menu*/
                                    _private.buildButton(selfmenu);
                                    SYS_MENU.push({label: name, evento: Tools.htmlEntities(v.evt_ajax), title: name, alias: v.alias, root: rot});
                                } else {
                                    /*si no tiene evento ajax, entonces tine subniveles*/
                                    _private.subItem(v.id_menu, oSettings, rot);
                                }
                    ...
                    $.each(oSettings.data, function (i, v) {
                        ...
                        if (v.parent == 0 && v.evt_ajax == 'not') {
                            if (JSON.stringify(cont).indexOf(v.id_menu) == -1) {
                                ...
                                li = `
                                <li>
                                    <a href="javascript:;" data-root="${name}" title="${name}"><i class="fa fa-lg fa-fw fa-home"></i> <span class="menu-item-parent text-menu-sm">${name}</span></a>
                                    <ul id="mnu_${v.id_menu}"></ul>
                                </li>`;
                                cont.push(v.id_menu);
                        ...
                        if (v.parent == 0 && v.evt_ajax != 'not') {
                            if (JSON.stringify(cont).indexOf(v.id_menu) == -1) {
                                ...
                                SYS_MENU.push({label: name, evento: Tools.htmlEntities(v.evt_ajax), title: name, alias: v.alias, root: name});
                            }
                        }
