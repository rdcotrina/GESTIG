...
* Descripcion : GrupoAjax.js 
* ---------------------------------------
*/ 
"use strict";
class GrupoAjax_ extends Ajax_ {

    constructor() {
        super();
        this._controller = 'admision/Grupo/';
        this._views = 'app/admision/views/grupo/';
        this._idGridGrupo = null;
    }
    ...
    formIndex(context){
        return super.send({
            ...
            final: function (obj) {/*se ejecuta una vez que se cargo el HTML en success*/
                obj.context.gridGrupo(context._alias);
            }
        });
    }
    
    gridGrupo(alias){
        $(`#${TABS.ADMGRPO}grid`).livianGrid({
            context: this,
            alias: alias,
            tScrollY: "200px",
            pDisplayLength: 25,
            ...
            tButtons:[{
                button: BTNSYS.NEW,                
                ajax: `Exe.GrupoDom.formNewGrupo`
            }],             
            ...
            sAxions: {
                buttons: [{
                    button: BTNSYS.EDT,
                    ajax: {
                        fn: "Exe.GrupoDom.formEditGrupo",
                        serverParams: "_1"
                    }
                }, {
                    button: BTNSYS.DEL,
                    ajax: {
                        fn: "Exe.GrupoDom.deleteGrupo",
                        serverParams: "_1"
                    }
                }]
            },
            ...
            fnCallback: function (oSettings) {
                oSettings.context._idGridGrupo = oSettings.tObjectTable;
            }
        });
    }
               
    newGrupo(btn, context, tk) {
        return super.send({
            token: tk,
            element: btn,
            context: this,
    ...
    postEdit(tk, contextDom) {
        return super.send({
            ...
            serverParams: function (sData, obj) {
                sData.push({name: '_pkGrupo', value: contextDom._key});
            }
        });
    }
    ...
    delete(btn, tk) {
        return super.send({
            ...
            dataType: 'json',
            serverParams: function (sData, obj) {
                sData.push({name: '_pkGrupo', value: obj.context._key});
            }
        });
    }
            
    findGrupo(contextDom) {
        super.send({
            flag: 1,
            token: _tk_,
            dataAlias: this._alias,
            context: this,
            root: `${this._controller}findGrupo`,
            dataType: 'json',
            serverParams: function (sData, obj) {
                sData.push({name: '_criterio', value: contextDom._key});
            },
            success: function (obj) {
                contextDom.setGrupo(obj.data);
            }
        });
    }
